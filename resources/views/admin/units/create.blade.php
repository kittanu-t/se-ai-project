...
<div class="container-fluid py-4" style="background-color: #f8f9fa; min-height: 100vh;">
    <div class="row justify-content-center">
        <div class="col-md-8">
            <h1 class="text-center text-dark mb-4">Add Unit — {{ $field->name }}</h1>

            <div class="card p-4 shadow-sm rounded-4">
                <form method="POST" action="{{ route('admin.fields.units.store',$field) }}" class="needs-validation">
                    @csrf
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" name="index" value="{{ old('index',1) }}" min="1" required class="form-control" id="index" style="border: 1px solid #6C757D; border-radius: 8px;">
                        @error('index')
                            <div class="text-danger small mt-1">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" name="capacity" value="{{ old('capacity',1) }}" min="0" required class="form-control" id="capacity" style="border: 1px solid #6C757D; border-radius: 8px;">
                        @error('capacity')
                            <div class="text-danger small mt-1">{{ $message }}</div>
                        @enderror
                    </div>
